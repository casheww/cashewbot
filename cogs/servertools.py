...
async def cash_setup(db, guild):
    info = json.dumps({'info': {'name': guild.name,
                                'join': str(dt.now().strftime(f'%d/%m/%Y'))}})
...
class ServerTools(commands.Cog):
    ...
    @commands.command(
        description="Resets ALL the data stored by the bot about the guild. "
        "Use with caution.",
    ...
    @commands.command(
        description="Returns stats and info regarding the current server.",
        aliases=[
            'serverinfo',
            'guild_info'],
